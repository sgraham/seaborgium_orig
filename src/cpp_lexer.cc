...
Lexer* MakeCppLexer() {
  ...
  classname_defs.AddWithTransition("[a-zA-Z_][a-zA-Z0-9_]*", Lexer::NameClass, Lexer::Pop);
  // TODO ?=> : classname_defs.AddWithTransition("\\s*(?=>)", Lexer::Text, Lexer::Pop);
  ...
  root_defs.Add("(\\d+\\.\\d*|\\.\\d+|\\d+)[eE][+-]?\\d+[LlUu]*", Lexer::LiteralNumberFloat);
  root_defs.Add("(\\d+\\.\\d*|\\.\\d+|\\d+[fF])[fF]?", Lexer::LiteralNumberFloat);
  ...
  // TODO(<type 'function'>)   root_defs.AddWithTransition("(class)(\\s+)", <function callback at 0x000000000237AEB8>, classname);
  root_defs.Add("(bool|int|long|float|short|double|char|unsigned|signed|void|wchar_t)\\b", Lexer::KeywordType);
  root_defs.Add("(_{0,2}inline|naked|thread)\\b", Lexer::KeywordReserved);
  root_defs.Add("__(asm|int8|based|except|int16|stdcall|cdecl|fastcall|int32|declspec|finally|int64|try|leave|wchar_t|w64|virtual_inheritance|uuidof|unaligned|super|single_inheritance|raise|noop|multiple_inheritance|m128i|m128d|m128|m64|interface|identifier|forceinline|event|assume)\\b", Lexer::KeywordReserved);
  root_defs.Add("(__offload|__blockingoffload|__outer)\\b", Lexer::KeywordPseudo);
  root_defs.Add("(true|false)\\b", Lexer::KeywordConstant);
  root_defs.Add("NULL\\b", Lexer::NameBuiltin);
  // TODO: ?!: : root_defs.Add("[a-zA-Z_][a-zA-Z0-9_]*:(?!:)", Lexer::NameLabel);
  ...
  string_defs.Add("\\\\([\\\\abfnrtv\"\\\']|x[a-fA-F0-9]{2,4}|[0-7]{1,3})", Lexer::LiteralStringEscape);
  string_defs.Add("[^\\\\\"\\n]+", Lexer::LiteralString);
  string_defs.Add("\\\\\\n", Lexer::LiteralString);
