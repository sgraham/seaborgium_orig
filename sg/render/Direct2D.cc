...
//{
	namespace Renderer
	{
		...
		void Direct2D::DrawFilledRect( Rect rect )
		{
			Translate( rect );
		...
		void Direct2D::SetDrawColor(Color color)
		{
		...
		bool Direct2D::InternalLoadFont( Font* pFont )
		{
		...
		void Direct2D::LoadFont( Font* pFont )
		{
			if ( InternalLoadFont( pFont ) )
				m_FontList.push_back( pFont );
		}

		void Direct2D::InternalFreeFont( Font* pFont, bool bRemove )
		{
			if ( bRemove )
		...
		void Direct2D::FreeFont( Font* pFont )
		{
			InternalFreeFont( pFont );
		}

		void Direct2D::RenderText( Font* pFont, Point pos, const string16& text )
		{
		...
		Point Direct2D::MeasureText( Font* pFont, const string16& text )
		{
			// If the font doesn't exist, or the font size should be changed
			if ( !pFont->data || fabs( pFont->realsize - pFont->size * Scale() ) > 2 )
			{
      ...
      (void)hr;
		...
		void Direct2D::DrawTexturedRect( Texture* pTexture, Rect rect, float u1, float v1, float u2, float v2 )
		{
		...
		void Direct2D::DrawTexturedRectAlpha( Texture* pTexture, Rect rect, float alpha, float u1, float v1, float u2, float v2 )
		{
			TextureData* pTexData = (TextureData*) pTexture->data;
		...
		bool Direct2D::InternalLoadTexture( Texture* pTexture )
		{
		...
		void Direct2D::LoadTexture( Texture* pTexture )
		{
			if ( InternalLoadTexture( pTexture ) )
				m_TextureList.push_back( pTexture );
		}

		void Direct2D::InternalFreeTexture( Texture* pTexture, bool bRemove )
		{
			if ( bRemove )
		...
		void Direct2D::FreeTexture( Texture* pTexture )
		{
			InternalFreeTexture( pTexture );
		}

		Color Direct2D::PixelColour( Texture* pTexture, unsigned int x, unsigned int y, const Color& col_default )
		{
			TextureData* pTexData = (TextureData*) pTexture->data;
		...
		void Direct2D::DrawLinedRect( Rect rect )
		{
			Translate( rect );
		...
		void Direct2D::DrawShavedCornerRect( Rect rect, bool bSlight )
		{
			Translate( rect );
