...
class ThreadContext {
 public:
  ...
  // Walk the callstack.
  virtual ThreadCallStack* GetCallStack(string16* err) = 0;
};
...
class Thread {
 public:
  ...
  // Suspends execution of this thread. If false is returned, |err| will be
  // filled out.
  virtual bool Suspend(string16* err) = 0;

  // Resumes execution of this thread. If false is returned, |err| will be
  // filled out.
  virtual bool Resume(string16* err) = 0;

  // Get the name of this thread if any. If false is returned, |err| will be
  // filled out.
  virtual bool GetName(string16* name, string16* err) = 0;
  ...
  // the thread context cannot be retrieved, NULL will be returned with |err|
  // filled out. The returned ThreadContext is only valid while the thread
  // stays in a suspended state.
  virtual ThreadContext* GetContext(string16* err) = 0;
};
...
class Process {
 public:
  ...
  // Get the name of this process. If false is returned, |err| will be
  // filled out.
  virtual bool GetName(string16* name, string16* err) = 0;
  ...
  // found. If NULL is returned, |err| will be filled with a user-presentable
  // error.
  virtual Thread* GetThread(ThreadId thread_id, string16* err) = 0;
  ...
  // retrieved. If NULL is returned, |err| will be filled with a
  // user-presentable error.
  virtual Heap* GetHeap(HeapId heap_id, string16* err) = 0;
};
...
class DebugConnection {
 public:
  ...
  // could not be created. In this case,  |err| will be filled out with a
  // user-presentable error.
  ...
  // this case, |err| will be filled out with a user-presentable error.
  virtual Process* ProcessAttach(ProcessId process_id, string16* err) = 0;
};
